...
NUM_COLUMNS = [ 'I'+str(x) for x in range(1,14)] + [ 'hour',]

CAT_COLUMNS = [ 'C'+str(x) for x in range(1,27)] + [ 'hour_c',]
...
def generate_feature(train_input_file, test_input_file,
    ...
    row_per_hour_trn = float(len(trn)) / HOURS_PER_DAY / DAYS_IN_TRN
    row_per_hour_tst = float(len(tst)) / HOURS_PER_DAY / DAYS_IN_TST

    print "row_per_hour_trn %f" % row_per_hour_trn
